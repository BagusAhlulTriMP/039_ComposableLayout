package com.example.compotable
...
import com.example.compotable.ui.theme.CompotableTheme
...
fun TampilText(message: String, from:String, modifier: Modifier=Modifier){
    Column(verticalArrangement = Arrangement.Center, modifier = modifier) {
        Text(
            color = Color.Blue, text = message, fontSize = 80.sp, lineHeight = 110.sp, textAlign = TextAlign.Center

        )
        Text(color = Color.Red, text = from, fontSize = 60.sp, modifier = Modifier
            .padding(16.dp)
...
fun GreetingPreview() {
    CompotableTheme {
       TampilLayar(pesan = "Selamat Pagi", dari = "UMY" )
    }
}